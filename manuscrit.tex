...
Nous implémentons, pour terminer, différentes stratégies de search pensées spécifiquement pour l'exploration des archives Web. Une fois que des documents ont matché une requête utilisateur, il s'agit, si ce dernier en fait la demande, de retourner les pages Web dont les archives sont les plus proches d'une date précise. Il est ainsi possible de préférer voir des pages collectées soit en amont, soit en aval ou soit autour de la date ciblée (Figure ???). Internet Archive par exemple utilise la dernière option dans la WayBack Machine.

Là aussi ...

Notre moteur de recherche est finalement déployé sur nos serveurs. Les indexes sont distribués entre plusieurs machines pour améliorer les temps de réponse et d'indexation du système. On appelle ainsi \textit{sharding} l'action de scinder un indexe en plusieurs sous indexes avant des les distribuer. Nous suivons une configuration classique \textit{master-slave}\footnote{\url{https://lucene.apache.org/solr/guide/6_6/solrcloud.html}} où l'instance maître de notre moteur de recherche centralise les réponses utilisateurs avant de les dispatcher entre ses instances esclaves qui, elles seules, sont habilitées à retourner des résultats. 
...
Et là on tente une requete et merde ... y'a un trout, bon ok on pensait bien qu'il pouvait y avoir des crawl blindess mais à ce point !

Les archives sont les traces directes du crawler et non du web (Cf mises en gardes précédentes) + exemple sur yabiladi.com 
...
donc il faut descendre au niveau de la page et y extraire d'autres temporalités, d'autres forme d'exploration qui ne dépendent pas non plus de la linéarité proposé par les moteurs d'exploration classique.
