...
\subsection{Un embrasement soudain}
...
Les membres du forum contribuent d'abord et avant tout aux threads nouvellement crées. Ils ne reviennent que dans de très infimes cas participer à des discussions passées, construisant leur trajet d'auteur d'un instant $t_i$ vers un instant $t_{i+1}$, sans retour arrière. Le forum est ainsi ressenti comme flux continu de nouveaux threads, demandant des utilisateurs qu'ils entretiennent une présence régulière sur le site. En cela, ils sont peu nombreux à marquer de longues pauses entre deux interventions, l'intervalle de temps maximum entre deux messages postés par un même auteur étant de 7~années. 

\begin{figure*}
  \includegraphics[width=\linewidth]{graphics/cluster-yabiladi}
  \caption{Spatialisation du graphe $G$ par Force Atlas et clustering des threads par classes de modularité}
  \label{fig:cluster-yabiladi}
\end{figure*} 

\noindent Nous utilisons, ensuite, un algorithme de clustering par classes de modularité\footnote{\url{https://fr.wikipedia.org/wiki/Modularité\_(réseaux)}} \citep{blondel_fast_2008}, pour grouper entre eux les threads du forum et voir, sous une nouvelle perspective, les points de fixations précédemment révélés par la figure \ref{fig:yabi-3}. La \textbf{modularité} d'un graphe est un score $\in [-1,1]$ qui traduit le rapport entre le nombre d'arêtes réel et le nombre d'arêtes espérée (en plaçant des arrêtes de manière aléatoire) d'un groupe de nœud. Plus ce score est élevé, plus ce groupe se rapproche topographiquement de la structure d'une communauté (Figure \ref{fig:cluster-yabiladi}).
...
Ce faisant, le graphe $G$ est divisé en 8 groupes distincts (Figure \ref{fig:cluster-yabiladi}, couleurs et numérotés), traduisant chacun un moment singulier de l'histoire de cette communauté.
