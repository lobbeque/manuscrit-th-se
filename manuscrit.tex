...
Dans notre imaginaire, le Web rime avec immédiateté : il est possible d'accéder à tout et à tout instant. Sous le régime d'un perpétuel présent, le Web se déploie ainsi à perte de vue et se laisse découvrir en continu. Mais, derrière la masse de données quotidiennement générées par et sur la toile, se dissimule pourtant un passé balbutiant qui ne demande qu'à être exploré. En effet, le Web possède déjà sa propre micro-histoire. Quelques années seulement après sa création, des pionniers se sont mis en tête d'archiver régulièrement les ressources publiées en ligne (les pages, les liens hypertextes, les contenus multimédias~\ldots{}) afin de sauvegarder pour les générations  futures une partie de cet héritage numérique. Aussi, devenu l'incarnation de l'accélération du temps et du rythme des échanges entre les êtres humains, le Web peut-il maintenant être l'objet de recherches historiques ? Peut-on faire un pas de côté, ralentir et s'offrir la possibilité d'un retour arrière sur le Web et sur son histoire ?\\

\noindent Ce faisant, la présente thèse ambitionne de se lancer dans l'exploration à grande échelle de vastes corpus d'archives Web, se situant à la frontière entre l'informatique, l'histoire et la sociologie. En effet, les enjeux ici sont multiples et dépassent souvent le cadre stricte d'une seule discipline.

Pour le chercheur en informatique, le volume et l'étendue des corpus à analyser pose un premier problème d'ingénierie et de mise en place de systèmes d'exploration capables d'interroger de manière efficace et rapide les archives Web. Mais ce qui sous-tend à cette question d'architecture est une réflexion plus large sur la structure de données des pages Web archivées. Doit-on conserver les archives telles que nous les restituent les archivistes ou peut-on imaginer une suite de transformations à même d'améliorer l'analyse ? Enfin, et c'est un sujet qui revient souvent dans ce genre projet pluridisciplinaire, comment faire dialoguer méthodes informatiques automatiques avec les pratiques de chercheurs venant de disciplines éloignées ? Comment traduire une problématique informatique en une question de recherche sociologique ? Comment tracer des ponts et des espaces de communication entre toutes ces disciplines ?

Pour l'historien 

Pour le sociologue 

Mais transversalement à tous ces enjeux nous proposons la problématique suivante ?

Et nous ne partons pas d'une page blanche 
...
Cette thèse porte ainsi sur l'exploration de corpus 

Cette thèse se siue donc au croisement de trois discipline Informatique, histoire et sociologie (double direction). Informatique car il existe déjà des outils appelée archives Web qui nécessite le dévellopement d'outils d'exploration et puis il y a cette enjeux interdisciplinaire de faire dialoguer des outils grande échelle et des méthodes plus quali ou des personens discitant avec un autre vocabulaire. Histoire car les archives Web reste une énigme, un lien innexplorer et nous tacherons de comprendre pourquoi, il y a aussi ce côté méthodo, comme faire une archéologie du Web, comment s'y prendre, comment écrire l'histoire depuis le Web. Sociologique car il nous faudra comprendre les tenant de l'impacte sociale du Web, caractériser les utilisateurs analyser, valider ... 

Nous ne partons pas d'une toile blanche et nous nous inscrivons dans l'éhirtage du projet e-Diasporas qui était déjà à la frontière de plein de trucs et qui a le bon gout de nous fournir une super archive (depuis les années 90, le Web a ...). Aussi partant de cela nous chercherons dans un premier temps à parler de moments singuliers d'événements 
Cette thèse = Comment parler d'un moment singulier dans la double temporalité du Web et du réel ?
