...
\noindent Tout au long de ce chapitre, nous nous sommes attelés à présenter, décrire et défricher les cas d'usage du fragment Web. 

Le terrain théorique ayant été préparé, dans le courant des années 2000, par les travaux pionniers de N. Brügger, il existait un espace analytique à explorer entre l'élément Web et la page Web. C'est dans cet entre-deux, que nous avons défini notre nouvelle unité d'exploration des archives Web : le fragment Web. Le fragment Web est un sous ensemble sémantique et syntaxique d'une page Web.

Avec le fragment Web, le chercheur retrouve une forme de souplesse vis à vis des archives Web. Il quitte les interfaces d'exploration vitrines et s'assoit à la table de montage. Là, la grammaire introduite par le fragment Web, lui permet de découper, déplacer et mettre en relation des éléments éparts, extraits de ses corpus archivés.  

Plus précises, historiquement parlant, les analyses portant sur les fragments Web s'appuient, d'abord et avant tout, sur leurs dates d'édi\-tion, plutôt que sur les seules dates de téléchargement des pages Web collectées. Ce faisant, les archives basculent d'une temporalité à l'autre. Elles quittent le temps des crawlers pour retrouver le temps du Web tel qu'il a été. Le chercheur accède à une mémoire antérieur au passage du crawler, mémoire jusqu'ici retenue dans les données des fichiers archivés. Le fragment témoigne ainsi directement du geste des auteurs, lecteurs et bloggers du Web passé et replace, de fait, l'humain au cœurs de l'étude des archives Web.

Intégré à un moteur d'exploration des archives Web, le fragment Web permet de réduire et d'atténuer considérablement les effets des artéfacts de crawl. Certains biais d'analyse disparaissent et de nouvelles modalité de recherches peuvent être testée, nous avons notamment insister sur la détection d'événements dans les archives. 

Mais, trouver une bonne méthode d'extraction des fragments Web, depuis les pages archivées, reste un champ de recherches pleinement ouvert. Sur ce point, nous apportons une première proposition, critiquable par son usage important d'heuristiques non génériques, mais qui selon nous correspond bien à notre méthodologie d'analyse. Nous souhaitons garder le contrôle sur les traitements que nous déléguons aux algorithmes et aux machines. Nous verrons, dans le chapitre suivant, en suivant le fil de deux explorations successives, que le Web nous oblige à mettre les mains dans les données, à tout le temps débrayer du qualitatif au quantitatif, de l'automatique au coup par coup, etc. 
