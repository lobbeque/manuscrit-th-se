...
\noindent Ces différents biais maintenant présentés, nous pouvons nous tourner vers la description de l'architecture de notre moteur d'exploration d'archives Web. Nos corpus étant en DAFF, il n'a pas forcément été possible de réutiliser des éléments open-source issus d'autres moteurs (quasiment tous conçus pour accueillir du WARC). De fait cette section est intéressante pour qui souhaite mettre en place ou comprendre dans le détail la mécanique d'une tel système. Il s'agira donc ici d'une description plutôt orientée ingénierie dont la majeure partie a fait l'objet d'une publication démonstration\footnote{Lobbé, Q. (2008), \textit{Revealing Historical Events out of Web Archives}, TPDL 2018}.

\subsection{Extraction et enrichissement}

les daff sont mangés dans un java, là ça va dans un hdfs puis dans un spark distribué où on join les data et méta et où on ajoute des infos venant de l'extérieure puis on envoie vers solr 

\subsection{Définition du schéma d'indexation}

bon y'a déjà des travaux et on a vue que l'indexation se faisait à la page (document based partition)

donc on va définir notre index 

mais d'abord c'est quoi un index ?

c'est quoi un schéma ?

à quoi correspond notre schéma ?
