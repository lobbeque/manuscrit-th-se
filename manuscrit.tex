...
\subsection{Topologie et genèse d'un site}

\noindent La mobilisation de la jeunesse marocaine sur les réseaux sociaux a servi d'amorce aux manifestations du 20 février 2011. Pour celles et ceux qui ne peuvent y participer, les plateformes Twitter et Facebook deviennent, l'espace d'une journée, une extension des rues de Casablanca, de Rabat et de Marrakech via les hashtags \textit{\#Feb20 \& \#20Fev} (Section \ref{sec:5_blogs}). Porté par une stratégie de communication numérique inédite, l'élan de ce mouvement parvient à atteindre la communauté des marocains de l'étranger \citep{desrues_mouvement_2012}. Parallèlement, le portail en ligne \textit{yabiladi.com}  s'impose au cœur de la diaspora marocaine comme un point de rassemblement et de discussion privilégié. Dès lors, se pose une question : comment les membres du forum de \textit{yabiladi.com} ont-il pu réagir à ces événements ? Peut on trouver dans nos archives, la trace de l'organisation de cette mobilisation et de la gestion de ces conséquences après coup ?

Attachés aux principes de l'AED, nous suivrons, à partir de ce point, une approche résolument visuelle. Aussi, commençons par définir notre espace d'exploration comme suit : un site Web unique (\textit{yabiladi.com}), archivé toutes les semaines entre Mars 2010 et Septembre 2014 par les équipes de l'INA (Section \ref{sec:3_edias}). Mais parmi la masse des données préservées, seuls les remous provoqués par les événements du 20 février 2011 doivent attirer, dans un premier temps, notre attention. Il nous faut définir un avant et un après 20 février, dans le cadre très précis du forum de \textit{yabiladi.com}, de ces pages et de ces threads de messages. Notre exploration se place donc à hauteur de ces messages et, à travers eux, des utilisateurs qui les ont publié. Pour ce faire, nous ne conservons, grâce à notre moteur d'exploration, que les fragments Web ayant la forme générique d'un message posté sur un forum : un nom d'auteur, une date d'édition et un contenu textuel publiés au fil d'un thread. 

Une fois ces fragments extraits et filtrés, nous pouvont formuler la problématique de visualisation suivante : Comment voir le processus d'agrégation d'un ensemble d'utilisateurs autour d'un événement particulier, et ce, à travers les messages qu'ils postent sur un forum ? Mais à y regarder de plus prêt, plusieurs pistes de réflexion sous tendent cette question : \textbf{1)} penser un site Web et sa structure comme un espace ou naviguent des utilisateurs \textbf{2)} penser l'évolution dans le temps de la structure de ce site \textbf{3)} penser le devenir d'un utilisateur comme un trajet passant d'un message ou d'une page à l'autre de ce site \textbf{4)} penser l'agrégation de plusieurs trajets autour d'un événements singulier. 

Pour résoudre ce problème, commençons par chercher une solution aux deux premières pistes de réflexion : Comment visualiser, partant d'un corpus d'archives Web, les changements topologiques d'un site Web au cours du temps ? Sur ce point, M. Toyoda et M. Kitsuregawa proposent de voir la genèse d'un site à travers une série temporelle de graphes \citep{toyoda_extracting_2003,toyoda_system_2005}, approche qui a servi d'inspiration à notre prototype \textit{en oursin} (Figure \ref{fig:prototypes}, a), présenté en section \ref{sec:3_constituer}. Mais, il nous semble difficile de comparer visuellement de telles séries surtout lorsque les sites (comme \textit{yabiladi.com}) se révèlent être très verbeux en URLs à afficher. 

À l'opposé, la méthode employée par B. Fry, dans son application Anemone\footnote{\url{http://benfry.com/anemone/}} \citep[p.76-82]{fry_organic_2000}, permet de saisir de manière dynamique l'évolution d'un site, vu ici comme un être vivant en pleine croissance. Le système, conçu par Fry, part de la première page archivée d'un site donné, à laquelle il ajoute toutes les $n$ secondes une nouvelle ramification, collant ainsi à l'arborescence du site tel qu'il a été collecté (figure \ref{fig:anemone}) :
...
\noindent De notre côté, deux dimensions (le site comme espace et le temps) servent également de cadre à l'étude d'un processus historique plus général (l'agrégation des utilisateurs autour du M20F). Aussi, définissons maintenant notre proposition de représentation.

\begin{figure}
  \includegraphics[width=\linewidth]{graphics/topo}
  \caption{Évolution de la structure d'un site fictif \textit{inter.net} dans le temps}
  \label{fig:topo}
\end{figure}

\noindent Pour visualiser l'évolution de la structure d'un site, nous choisissons de conserver le temps comme dimension d'analyse, placé en abscisse à la manière d'une timeline. Au dessus de cet axe, le site se déploie sous la forme d'un arbre d'URLs, grandissant à mesure que de nouvelles pages y sont publiées, comme l'illustre la figure \ref{fig:topo}. 

Chaque URL du site représente une branche de l'arbre, dont les feuilles sont les pages Web (Figure \ref{fig:topo}, points rouges). L'abscisse d'une URL est déterminée suivant la date de création (dans le meilleur des cas) ou de téléchargement (dans le pire des cas) de la page qu'elle adresse. Pour positionner les embranchements mécaniques\footnote{les portions d'URL entre deux "\textit{/}" ne correspondant à aucune page réelle} de notre arbre, nous leur attribuons arbitrairement une date médiane (Figure \ref{fig:topo}, $t(p_2) - t(p_1)$). L'ordonnée d'une URL est d'abord fonction de son âge (du haut vers le bas, de la plus ancienne à la plus récente) qui sert ensuite d'encrage sur un demi-cercle partant de l'embranchement parent (Figure \ref{fig:ordonnee}).
...
L'épaisseur d'une branche de l'arbre d'URLs est fonction du nombre d'enfants à chaque embranchement. Les pages Web utiles à l'étude sont indiquées par des points rouges dont la taille est proportionnelle au nombre de messages postés dessus. Nous développons ensuite un système\footnote{Open-source et téléchargeable ici \url{https://github.com/lobbeque/archive-viz/tree/master/stayingAlive}} permettant de récupérer, depuis notre moteur d'exploration et depuis Internet Archive, l'ensemble des URLs archivées d'un site donné, que nous représentons à travers une interface de visualisation Web. L'utilisateur peut zoomer à volonté dans l'arbre et sélectionner le détail d'une branche s'il le souhaite (Figure \ref{fig:reve}).

\begin{figure*}
  \includegraphics[width=\linewidth]{graphics/reve}
  \caption{Aperçut de notre interface pour visualiser l'évolution topographique d'un site, ici pour la section \textit{Interprétation des rêves, roqya, djinn} du forum de \textit{yabiladi.com}}
  \label{fig:reve}
\end{figure*}

\noindent Ceci étant définit, revenons maintenant à \textit{yabiladi.com} et à la manifestation du 20 février 2011. L'arbre d'URLs qui servira de support à la visualisation est construit à partir des 2,683,928 pages archivées de notre corpus et des 887,981 pages des collectages d'Internet Archives. L'idée étant, ici, de proposer une représentation, la complète possible, de la structure du site et de son évolution. Donc nous associons et mélangeons plusieurs sources de données entre elles. Les pages venant de nos archives sont associées, si possible, à une date de création; les pages fournies par Internet Archives sont, elles, datées par rapport à leurs dates de collecte. En cas de doublon d'un corpus à l'autre, nous privilégions la datation la plus ancienne.

Les archives DAFF de \textit{yabiladi.com} sont alors envoyées dans notre moteur d'exploration qui les fragmente et les indexe. Là, un filtre est appliqué pour ne conserver que les pages appartenant à la seule section forum du site, celles contenant la mention "\textit{/forum/}" dans leurs URLs. À ce niveau, nous obtenons 109,534 pages associées à une date de création et segmentées en 422,906 fragments Web.

Il ne nous semble pas nécessaire, dans un premier temps, d'analyser toutes les catégories du forum, les sections \textit{Général} et \textit{Actualités} étant plus susceptibles de renfermer des threads et des messages associés au 20 février. Or, depuis l'analyse de la figure \ref{fig:categories}, nous savons que la section \textit{Actualités} a subi de nombreux changements au cours du temps : \textit{Actualités}, \textit{Actualités du Maroc}, \textit{Actualités Marocaines},  \textit{Actualités internationales} et \textit{Actualité du Maroc et du Monde}. Aussi, nous choisissons d'agréger ces diverses entités en une seule et même section \textit{Actualités}. Par ailleurs, la structure des URLs du forum est ainsi faite, qu'il est possible de construire un nouveau filtre par catégories. En effet, depuis la création du forum en 2002, les URLs suivent le même schéma~:

\begin{figure}
  \includegraphics[width=\linewidth]{graphics/filtre}
\end{figure}  

\noindent Par exemple, la \textit{category\_id} de la section \textit{Général} est $1$. Nous appliquons donc ce filtre et obtenons en sortie de notre moteur les résultats suivants :
...
\noindent La table \ref{tab:forum} présente diverses statistiques issues de l'analyse de ces données, rappelons qu'un \textit{message} est ici un fragment Web ayant la forme supposée d'un message posté sur un forum et qu'un thread est une suite de messages associés à un même sujet de conversation. 

Pour identifier les messages\footnote{Le forum est écrit en majorité en français} se référant aux événements du 20 février 2011, nous utilisons la recherche plein texte de notre moteur et partons de l'ensemble de mots clés suivants :
...
\noindent Nous agrégeons, ensuite, par thread les fragments retournés. Après une validation manuelle, Nous n'en conservons que 12, selon qu'ils traitent directement du 20 février ou de ses conséquences. Nous présentons, ci dessous, la liste des différents titres (ou sujets) de ces 12 threads~:
...
\noindent Au final, nous obtenons un nouveau groupe de 343 threads appelé $V_1$. La figure \ref{fig:yabi-1} donne à voir la répartition de ces threads (points rouges) dans le temps, parmi les pages archivées du forum de \textit{yabiladi.com}.

\subsection{Agréger les contributeurs}

\noindent Il manque, à la figure \ref{fig:yabi-1}, une information importe : celle du trajet d'un utilisateur naviguant d'un thread à l'autre du forum. En définissant le fragment Web (Section \ref{sec:4_derrida}), nous souhaitions réintégrer de l'humain au cœur des archives Web, revenir aux gestes de celles et ceux qui alimentent quotidiennement les sites collectés.  

\begin{marginfigure}%
  \includegraphics[width=\linewidth]{graphics/thread}
  \vspace*{0.2cm}  
  \caption{Détail du thread \textit{J'ai besoin de vous ...}, les messages d'un auteur sont sélectionnés (bleu), les threads auxquels il a participé sont listés (bas)}
  \label{fig:thread}
\end{marginfigure} 

Or, les fragments Web que nous manipulons sont ici associés aux noms des auteurs qui les ont écrits. Il est ainsi possible de constituer des trajets entre fragments Web, suivant la trace laissée par les utilisateurs lorsqu'ils contribuent, par leurs messages, aux threads qui les intéressent. Nous modifions donc notre système de visualisation pour afficher de manière dynamique le trajet d'un utilisateur donné (Figure \ref{fig:thread}). La figure \ref{fig:yabi-2} présente l'un de ces trajets, agrégé au niveau des threads. Celui d'un utilisateur ayant écrit sur le forum d'avril 2007 à septembre 2011, contribuant par là même à 66 threads différents. Plus précisément, ce dernier a posté des messages dans les threads \textit{Qui assistera le 20 février au Maroc....???} et \textit{Le "20 Fevrier" est il democratique}. 
...
Mais c'est à un collectif que nous nous intéressons ici, pas à un individu isolé. Or, le fragment Web nous permet justement de conjuguer le devenir historique de multiples lignes processuelles (Section \ref{sec:4_derrida}), afin de raisonner autour de moments singuliers, à l'image du 20 février 2011. Pour ce faire, nous définissons maintenant $E_1$ comme l'ensemble des chemins empruntés par le groupe d'utilisateurs $E_0$ et le graphe $G=(V_1,E_1)$ comme un réseau de threads liés entre eux par les trajets de contributeurs communs. 

Afin de faciliter l'analyse de l'ensemble de ces chemins, nous les représentons en utilisant la méthode \textit{edge bundling} \citep{holten_hierarchical_2006}, qui permet de modifier localement la courbure d'un lien dans un graphe, pour le rapprocher visuellement de ces voisins. La figure \ref{fig:yabi-3} représente le graphe $G$ et donne à voir la manière dont il s'est construit dans le temps. Pour clarifier la lecture de cette figure, nous dissimulons l'ossature de l'arbre d'URLs \textit{yabiladi.com} et affichons (en noir) les trajets $E_1$ parmi les threads $V_1$ (en rouge).
